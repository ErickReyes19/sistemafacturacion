...
import { z } from "zod";
import { createCaja, updateCaja } from "../actions"; // Tu función para enviar datos
import { CajaSchema } from "../schema"; // Tu esquema de Zod para categoria
import { toast } from "sonner";

export function CajaFormulario({
  isUpdate,
  initialData,
}: {
  isUpdate: boolean;
  initialData: z.infer<typeof CajaSchema>;
}) {
  const router = useRouter();

  // Usamos Zod para resolver la validación
  const form = useForm<z.infer<typeof CajaSchema>>({
  ...
  // console.log("isValid");
  // console.log(isValid);
  async function onSubmit(data: z.infer<typeof CajaSchema>) {
    const cajaData = {
      caja: data,
    };
    ...
    const promise = async () => {
      if (isUpdate) {
      await updateCaja(cajaData.caja.id!, cajaData.caja);
      return { name: "Actualización" };
      } else {
      await createCaja(cajaData.caja);
    ...
    try {
      ...
      error: "Error al guardar la caja",
      });
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="col-span-1 sm:col-span-1">
                ...
                <FormDescription>
                  Por favor ingresa el nombre de la caja
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
