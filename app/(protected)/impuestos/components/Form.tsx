...
import { z } from "zod";
import { createImpuesto, updateImpuesto } from "../actions"; // Tu función para enviar datos
import { ImpuestoSchema } from "../schema"; // Tu esquema de Zod para categoria
import { toast } from "sonner";

export function ImpuestoFormulario({
  isUpdate,
  initialData,
}: {
  isUpdate: boolean;
  initialData: z.infer<typeof ImpuestoSchema>;
}) {
  const router = useRouter();

  // Usamos Zod para resolver la validación
  const form = useForm<z.infer<typeof ImpuestoSchema>>({
  ...
  // console.log("isValid");
  // console.log(isValid);
  async function onSubmit(data: z.infer<typeof ImpuestoSchema>) {
    const impuestoData = {
      impuesto: data,
    };
    ...
    const promise = async () => {
      if (isUpdate) {
        await updateImpuesto(impuestoData.impuesto.id!, impuestoData.impuesto);
        return { name: "Actualización" };
      } else {
        await createImpuesto(impuestoData.impuesto);
    ...
    try {
      await toast.promise(promise, {
        ...
        error: "Error al guardar el impuesto",
      });
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="col-span-1 sm:col-span-1">
                ...
                <FormDescription>
                  Por favor ingresa el porcentaje de la moneda.
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
